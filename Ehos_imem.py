...
cur = conn.cursor()
cur.execute("""CREATE TABLE if not exists employe(
          employee integer,  
          name text,
          lastname text,
          date text,
...
class UI_EmployessWindow(QMainWindow):
    ...
    def clicker_save(self):
        ...
        employee = self.line_employee.text()  
        name = self.line_name.text()
        lastname = self.line_lastname.text()
        date = self.line_date.text()
        gender = self.line_gender.text()
        phone = self.line_phone.text() 
        expertise = self.line_expertise.text() 
        part = self.line_part.text()
        
        
        if (employee == "" or name == "" or lastname == "" or date == "" or part == "" or gender == "" or phone == "" or expertise == ""):
            ...
            cur = conn.cursor()
            insert_to_tab = """INSERT INTO employe
                        (employee, name, lastname, date, gender, phone, expertise, part)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?);"""  # Fixed column name


            # Execute the insert statement with the values
            cur.execute(insert_to_tab, (int(employee), name, lastname, date, gender, int(phone), expertise, int(part)))
