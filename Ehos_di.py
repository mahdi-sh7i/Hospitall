...
class UI_ExtractionWindow(QMainWindow):
    def __init__(self):  # Corrected method name to __init__
        super(UI_ExtractionWindow, self).__init__()  # Corrected superclass initialization
        self.center()  # Call to center method
        # Load the ui file
        uic.loadUi("Ehos_di.ui", self) 
        # -------------------------------------------- 
...
###############################################3
    def openEmployessWindow(self):
        from Ehos_diem import UI_EmployessWindow  # Local import to avoid circular import
        self.window = UI_EmployessWindow()  # Create an instance of the main window
        self.window.show()  # Show the window (no exec_ needed for QMainWindow)

    def openPatientsWindow(self):
        from Ehos_dipa import UI_PatientsWindow  # Local import to avoid circular import
        self.window = UI_PatientsWindow()  # Create an instance of the main window
        self.window.show()  # Show the window (no exec_ needed for QMainWindow)

    def openDeptmentWindow(self):
        from Ehos_dide import UI_DeptmentWindow  # Local import to avoid circular import
...
if __name__ == "__main__":  # Corrected check to ensure this runs only when executed directly
    ...
    UIWindow = UI_ExtractionWindow()  # Create an instance of UI_AdmisionWindow
